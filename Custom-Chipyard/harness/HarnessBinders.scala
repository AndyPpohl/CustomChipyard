...
class WithGPIOPinsTiedOff extends HarnessBinder({
  case (th: HasHarnessInstantiators, port: GPIOPinsPort, chipId: Int) => {
    port.io := DontCare
    println("damn [RESET-BINDER] WithAXIResetMMIO class loaded (top-level)")
...
class WithBlackBoxSimMem(additionalLatency: Int = 0) extends HarnessBinder({
  case (th: HasHarnessInstantiators, port: AXI4MemPort, chipId: Int) => {
    //
    //require(false, s"SIMDRAM BINDER FIRED for chipId=$chipId, port=$port")
    ...
    withClock(port.io.clock) {
      val dramResetSync0 = RegNext(th.harnessBinderReset, true.B)
      val dramResetSync1 = RegNext(dramResetSync0, true.B)
      val dramReset = dramResetSync1
      mem.io.reset := dramReset.asAsyncReset
    }

    withClock(port.io.clock) {
      val coldResetSync0 = RegNext(th.extraResetForHarness, false.B)
      val coldResetSync1 = RegNext(coldResetSync0, false.B)
      val coldReset = coldResetSync1
      mem.io.cold_reset := coldReset
    }
    //val coldReset = th.extraResetForHarness
    //mem.io.cold_reset := coldReset
    ...
    port.io match {
      case io: DecoupledPhitIO => {
        ...
        withClock(clock) {
          ...
          val tsiResetSync0 = RegNext(th.harnessBinderReset, false.B)
          val tsiResetSync1 = RegNext(tsiResetSync0, false.B)
          val tsiReset = tsiResetSync1
          ...
          val success = SimTSI.connect(ram.io.tsi, clock, tsiReset, coldReset, chipId)
...
class WithSimTSIToUARTTSI extends HarnessBinder({
  case (th: HasHarnessInstantiators, port: UARTTSIPort, chipId: Int) => {
    ...
    val success = SimTSI.connect(Some(TSIIO(serial_width_adapter.io.wide)), th.harnessBinderClock, th.harnessBinderReset, false.B)
    when (success) { th.success := true.B }
...
/*
class WithTLResetHarnessConnect extends HarnessBinder({
  case (th: TestHarness, dut: chipyard.ChipTop, _) =>
    th.io.tlReset := dut.tlResetOut
})
*/

class WithAXIResetMMIO extends HarnessBinder({
  println("DEBUG: WithAXIResetMMIO binder loaded")

  {
    case (th: HasHarnessInstantiators, port , chipId)  =>
      //require(false, "fuckshit TLRESET binder fired")
      println(s"[BINDER]: saw port: ${port.getClass}")
  }
})  

/*
class WithAXIResetMMIO extends HarnessBinder({
  // This runs when the binder is constructed
  println("[DEBUG] WithAXIResetMMIO binder loaded")
  //println(s"fuck [DEBUG] Binder sees ports: " + th.harnessInstantiators.map(_.toString))

  // This is the actual binder partial function
  {
    //case (th: HasHarnessInstantiators, port: TLMemPort, chipId: Int) =>
    case (th: HasHarnessInstantiators, port: AXI4MemPort, chipId: Int) =>
      implicit val p: Parameters = th.p

      require(false, "TLRESET binder fired")

      //val resetMod = Module(new chipyard.harness.ResetAXIModule)
      val resetMod = Module(new chipyard.harness.ResetAXIModule)

      resetMod.io.a_valid := false.B 
      resetMod.io.a_ready := false.B 
      resetMod.io.a_addr := 0.U 
      resetMod.io.a_opcode := 0.U

      if(port.io.size > 0) {
        val tl = port.io(0)

        resetMod.io.a_valid := tl.a.valid
        resetMod.io.a_ready := tl.a.ready
        resetMod.io.a_addr  := tl.a.bits.address
        resetMod.io.a_opcode := tl.a.bits.opcode
        //resetMod.io.w_valid  := tl.valid
        //resetMod.io.w_ready  := tl.ready
      }
      printf("[TLRESET] port.io.size = %d\n", port.io.size.U)

      th.asInstanceOf[TestHarness].mmioResetReg :=
        th.asInstanceOf[TestHarness].mmioResetReg || resetMod.io.resetReq
  }
})
*/
...
class WithAXIResetMMIO extends HarnessBinder({
  ...
  case (th: HasHarnessInstantiators, port: TLMemPort, chipId: Int) =>
    ...
    val tl2axi = LazyModule(new TLToAxi4(AXI4SlavePortParameters(
      slaves = Seq(AXI4SlavePortParameters(
        address = port.params.base,
        resources = new Device("reset-mmio", Seq("reset-mmio")).reg("mem"),
        regionType = RegionType.UNCACHED,
        executable = true,
        supportsRead = TransferSizes(1, 8),
        supportsWrite = TransferSizes(1, 8)
      )),
      beatBytes = port.params.beatBytes
    )))

    port.io := tl2axi.node

    //val resetMod = Module(new chipyard.harness.ResetAXIModule(port.edge.bundle))
    val resetMod = Module(new chipyard.harness.ResetAXIModule(tl2axi.node.out.head._2.bundle))
    resetMod.io.axi <> tl2axi.node.out.head._2.bundle
    //resetMod.io.axi <> port.io.bits

    // OR into your TestHarness reset register
    th.asInstanceOf[TestHarness].mmioResetReg :=
      th.asInstanceOf[TestHarness].mmioResetReg || resetMod.io.resetReq
})
*/
...
*/
/*
class WithResetMMIO extends HarnessBinder({
  case (th: HasHarnessInstantiators, port: AXI4MMIOPort, chipId: Int) =>
    //implicit val ip = port.p
    implicit val p = th.p
    
    val resetMMIO = LazyModule(new ResetMMIO)
    val mm = Module(resetMMIO.module)

    resetMMIO.node := port.io
    //port.io := TLFragmenter(8,64) := resetMMIO.node

    th.asInstanceOf[TestHarness].mmioResetReg :=
      th.asInstanceOf[TestHarness].mmioResetReg || mm.io.resetReq

})

  
    dut.system.pbus.coupleTo("reset-mmio") { bus =>
      resetMMIO.node := TLFragmenter(8,64) := bus
      }
    
    val mm = Module(resetMMIO.module)

    val baseReset = th.referenceReset

    val combinedReset = (baseReset.asBool || mm.io.resetReq).asAsyncReset

    dut.module.reset := combinedReset
})


class WithIntermittentFullSystemReset extends HarnessBinder({
  case (th: HasHarnessInstantiators, port: ChipyardResetPort, chipId: Int) =>
    val combined = th.referenceReset.asBool || th.intermittent_reset
    port.io := combined.asAsyncReset
})
*/
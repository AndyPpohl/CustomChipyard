...
abstract trait HasHarnessInstantiators {
  ...
  //def extraResetForHarness: Bool
  //def powerOnResetForHarness: Reset
  val extraResetForHarness = Wire(Bool())
  val powerOnResetForHarness: Reset

  println("### HasHarnessInstantiators: elaborating ###")
  ...
  def instantiateChipTops(): Seq[LazyModule] = {
    ...
    val binderResetReg = RegInit(true.B)
    when (cycle > 1000.U) { binderResetReg := false.B }
    harnessBinderReset := (binderResetReg || extraReset)
    ...
    */
    ...
    withClockAndReset(harnessBinderClk, powerOnResetForHarness) {
      val cycle = RegInit(0.U(64.W))
      val fired = RegInit(false.B)
      val extraResetReg = RegInit(false.B)

      cycle := cycle + 1.U

      when (!fired && (cycle === 150000.U)) {
      //when(cycle === 84000.U) {
        extraResetReg := true.B
        fired := true.B
        printf("reset_fired at cycle %d\n", cycle)
      }.elsewhen (extraResetReg && cycle === 180000.U) {
        extraResetReg := false.B
        printf("reset_released at cycle %d\n", cycle)
      }

      extraResetForHarness := extraResetReg
  
      when (cycle(15,0) === 0.U) {
        printf(p"[HARNESS] cycle=$cycle, reset=${powerOnResetForHarness}\n")
      }
    }

    withClockAndReset(harnessBinderClk, powerOnResetForHarness) {
      val binderResetReg   = RegInit(true.B)
      val binderCycle      = RegInit(0.U(32.W))
      val extraResetSync0  = RegNext(extraResetForHarness, false.B)
      val extraResetSync1  = RegNext(extraResetSync0, false.B)

      binderCycle := binderCycle + 1.U
      when (binderCycle > 1000.U) {
        binderResetReg := false.B
      }

      //harnessBinderReset := binderResetReg || extraResetSync1
      harnessBinderReset := binderResetReg
    }

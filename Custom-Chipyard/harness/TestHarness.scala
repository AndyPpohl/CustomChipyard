...
class TestHarness(implicit val p: Parameters) extends Module with HasHarnessInstantiators {
  ...
  override def referenceReset = (reset.asBool || extraResetForHarness).asAsyncReset

  override val powerOnResetForHarness: Reset = reset.asAsyncReset
  ...
  when (!fired && cycle === 80000.U) {
    extraReset := true.B
    fired  := true.B
    printf("reset_fired\n")
    println(s"shitfuckpiss")
  }.elsewhen (extraReset && cycle === 81000.U) { // hold for 100 cycles
    extraReset := false.B
    printf("reset_released\n")
    println(s"shitfuckpiss2")
  }
  */
  //////////////////////////////////

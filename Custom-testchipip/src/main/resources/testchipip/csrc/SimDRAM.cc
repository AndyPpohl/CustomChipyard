...
static uint8_t* simdram_data = nullptr;
static size_t simdram_mem_base = 0;
...
extern "C" void simdram_reload_program() {
    if (loadmem_file != "" && simdram_data) {
        reg_t entry = 0;

        class reload_memif_t : public memif_t {
        public:
            reload_memif_t(uint8_t* _data, size_t _start)
                : memif_t(nullptr), data(_data), start(_start) {}
            void write(addr_t taddr, size_t len, const void* src) override {
                addr_t addr = taddr - start;
                memcpy(data + addr, src, len);
            }
            void read(addr_t, size_t, void*) override { assert(false); }
            endianness_t get_target_endianness() const override {
                return endianness_little;
            }
        private:
            uint8_t* data;
            size_t start;
        } memif(simdram_data, simdram_mem_base);

        load_elf(loadmem_file.c_str(), &memif, &entry, 0);
    }
}
...
{
    //printf("[MEM] memory_tick entered: reset=%d cold_reset=%d\n", reset, cold_reset);
    ...
    //    printf("[MEM] reset change: prev=%d now=%d\n", prev_reset, reset);
    //}

    if(cold_reset && !prev_reset) {
      printf("[MEM] cold reset edge detected, calling request_cold_reset()\n");
      request_cold_reset();
    }

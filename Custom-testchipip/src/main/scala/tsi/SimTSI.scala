...
object SimTSI {
  def connect(tsi: Option[TSIIO], clock: Clock, reset: Reset, coldReset: Bool, chipId: Int = 0): Bool = {
  //def connect(tsi: Option[TSIIO], clock: Clock, reset: Reset, chipId: Int = 0): Bool = {
    val exit = tsi.map { s =>
      val sim = Module(new SimTSI(chipId))
      sim.io.clock := clock
      sim.io.reset := reset
      sim.io.cold_reset := coldReset
      sim.io.tsi <> s
      sim.io.exit
    }.getOrElse(0.U)
...
class SimTSI(chipId: Int) extends BlackBox(Map("CHIPID" -> IntParam(chipId))) with HasBlackBoxResource {
  val io = IO(new Bundle {
    ...
    val cold_reset = Input(Bool())
    val tsi = Flipped(new TSIIO)
    val exit = Output(UInt(32.W))
  })

  addResource("/testchipip/csrc/cold_reset_globals.cc")
  addResource("/testchipip/csrc/cold_reset_globals.h")

...
#--------------------------------------------------
#  Notes from observations of arty constraints    #
#--------------------------------------------------

#most of the pins set are just devoted to setting the LED ports
#Im not sure if the LED ports are actually useful for our purposes or not
#it seems kind of safe to assume that they arent

#Always have a package pin set with the name and then
#we have an IOSTANDARD that set LVCMOS33 with that port we just set, i.e.
#set_property PACKAGE_PIN {E3} [get_ports {sys_clock}]
#set_property IOSTANDARD {LVCMOS33} [get_ports {sys_clock}]

#UART and JTAG make up all the other ports specified
#need to look into what that fanout and fanin thing they have going on is

#What is IOB and the PULLUP for? Why do those ones get set differently from the others?

#I think I'd maybe be able to either declare a lot of these with the same names
#in the zedboard.xdc and still have them work in the harness without changing those names

#What is the deal with the memory and the reset?
#Nothing for memory specified in the arty constraints so it is assumedly dealt
#with in some other manner
#Reset pin is a mystery, it looks like they want one to be set in the harness but
#then nothing is said about it in the actual configuration, why is that?

set_property PACKAGE_PIN Y9 [get_ports {sys_clock_p}]
set_property PACKAGE_PIN Y8 [get_ports {sys_clock_n}]
set_property IOSTANDARD DIFF_SSTL15 [get_ports {sys_clk_p sys_clk_n}]
set_property DIFF_TERM TRUE [get_ports {sys_clock_p sys_clock_n}]
create_clock -add -name sys_clk -period 5.000 [get_ports sys_clock_p]

#set_property IOSTANDARD {LVCMOS33} [get_ports {sys_clock_p}]
#set_property PACKAGE_PIN Y8 [get_ports {sys_clock_n}]
#set_property IOSTANDARD {LVCMOS33} [get_ports {sys_clock_n}]
#create_clock -period 10.000 [get_ports {sys_clock}] ;# 100 MHz

#set_property PACKAGE_PIN D18 [get_ports {sys_clock_n}]
#set_property IOSTANDARD {LVCMOS33} [get_ports {sys_clock_n}]
#set_property PACKAGE_PIN C19 [get_ports {sys_clock_p}]
#set_property IOSTANDARD {LVCMOS33} [get_ports {sys_clock_p}]

set_property PACKAGE_PIN F7 [get_ports sys_clock_p]
set_property PACKAGE_PIN E7 [get_ports sys_clock_n]
#set_property IOSTANDARD LVDS_25 [get_ports {sys_clock_p sys_clock_n}]
#set_property DIFF_TERM TRUE [get_ports {sys_clock_p sys_clock_n}]
#create_clock -add -name sys_clk -period 5.000 [get_ports sys_clock_p]


#set_property BOARD_PIN {G17} [get_ports {reset}]
#assign a push button to serve as reset spot
set_property PACKAGE_PIN {V19} [get_ports {reset}]
set_property IOSTANDARD {LVCMOS33} [get_ports {reset}]
set_property PULLDOWN {TRUE} [get_ports {reset}]

#setting LEDs
set_property PACKAGE_PIN T22 [get_ports {led_0}];  # "led_0"
set_property IOSTANDARD {LVCMOS33} [get_ports {led_0}]
set_property PACKAGE_PIN T21 [get_ports {led_1}];  # "led_1"
set_property IOSTANDARD {LVCMOS33} [get_ports {led_1}]
set_property PACKAGE_PIN U22 [get_ports {led_2}];  # "led_2"
set_property IOSTANDARD {LVCMOS33} [get_ports {led_2}]
set_property PACKAGE_PIN U21 [get_ports {led_3}];  # "led_3"
set_property IOSTANDARD {LVCMOS33} [get_ports {led_3}]
set_property PACKAGE_PIN V22 [get_ports {led_4}];  # "led_4"
set_property IOSTANDARD {LVCMOS33} [get_ports {led_4}]
set_property PACKAGE_PIN W22 [get_ports {led_5}];  # "led_5"
set_property IOSTANDARD {LVCMOS33} [get_ports {led_5}]
set_property PACKAGE_PIN U19 [get_ports {led_6}];  # "led_6"
set_property IOSTANDARD {LVCMOS33} [get_ports {led_6}]
set_property PACKAGE_PIN U14 [get_ports {led_7}];  # "led_7"
set_property IOSTANDARD {LVCMOS33} [get_ports {led_7}]

#setting up UART with jd pmod
set_property PACKAGE_PIN {W7} [get_ports {uart_rxd}]
set_property IOSTANDARD LVCMOS33 [get_ports {uart_rxd}]
set_property IOB {TRUE} [get_cells -of_objects [all_fanout -flat -endpoints_only [get_ports {uart_rxd}]]]

set_property PACKAGE_PIN {V7} [get_ports {uart_txd}]
set_property IOSTANDARD LVCMOS33 [get_ports {uart_txd}]
set_property IOB {TRUE} [get_cells -of_objects [all_fanin -flat -startpoints_only [get_ports {uart_txd}]]]

#Setting TCK
set_property PACKAGE_PIN {V4} [get_ports {jtag_TCK}]
set_property IOSTANDARD LVCMOS33 [get_ports {jtag_TCK}]
set_property PULLUP {TRUE} [get_ports {jtag_TCK}]
set_property CLOCK_DEDICATED_ROUTE {FALSE} [get_nets [get_ports {jtag_TCK}]]

set_property PACKAGE_PIN {V5} [get_ports {jtag_TMS}]
set_property IOSTANDARD LVCMOS33 [get_ports {jtag_TMS}]
set_property PULLUP {TRUE} [get_ports {jtag_TMS}]

set_property PACKAGE_PIN {W5} [get_ports {jtag_TDI}]
set_property IOSTANDARD LVCMOS33 [get_ports {jtag_TDI}]
set_property PULLUP {TRUE} [get_ports {jtag_TDI}]

set_property PACKAGE_PIN {W6} [get_ports {jtag_TDO}]
set_property IOSTANDARD LVCMOS33 [get_ports {jtag_TDO}]
set_property PULLUP {TRUE} [get_ports {jtag_TDO}]

#these use the og UART pins
set_property PACKAGE_PIN {Y5} [get_ports {uart_tsi_rxd}]
...
set_property PACKAGE_PIN {AB1} [get_ports {uart_tsi_txd}]
...
# UART - Bank 13
# ---------------------------------------------------------------------------- 
#set_property PACKAGE_PIN AB1 [get_ports {UART_TX}];  # "AC-ADR0"
#set_property PACKAGE_PIN Y5  [get_ports {UART_RX}];  # "AC-ADR1"


#UART seems to be a bit of a mystery, UARTs in the Arty Master looks like this
#set_property -dict { PACKAGE_PIN D10   IOSTANDARD LVCMOS33  } [get_ports { uart_rxd_out }]; #IO_L19N_T3_VREF_16 Sch=uart_rxd_out
#set_property -dict { PACKAGE_PIN A9    IOSTANDARD LVCMOS33  } [get_ports { uart_txd_in }]; #IO_L14N_T2_SRCC_16 Sch=uart_txd_in
#but there's a ton of different UART pins set in the config.xdc for some reason
#All the UART that aren't the actual UART in the master are jd pin spots
#same goes for all the JTAG pins, they're all jd pin spots
